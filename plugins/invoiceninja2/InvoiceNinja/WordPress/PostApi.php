...
class PostApi
{
    ...
    public function enqueueStyles()
    {
        ...
        // Check if $post is set and has the post_type property
        if ( isset( $post ) && isset( $post->post_type ) && $post->post_type == 'invoiceninja_product' ) {
        ...
        if ( get_the_ID() == get_option( 'invoiceninja_product_page_id' ) ) {
            ...
            $color = '#0000EE';
            $profile = json_decode( get_option( 'invoiceninja_profile' ) );
            if ( isset( $profile->settings->primary_color ) ) {
                $color = $profile->settings->primary_color;
            }
        ...
        // Enqueue frontend styles and scripts if not in the admin area
        if ( ! is_admin() ) {
